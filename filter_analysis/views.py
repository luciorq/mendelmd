        ...
        if request.method == 'GET':
            ...
            if export != '':
                if export == 'csv':
                    ...
                    response['Content-Disposition'] = 'attachment; filename="Mendel,MD Variants Export.csv"'
                    writer = csv.writer(response)
                    
                elif export == 'txt':
                    response = HttpResponse(content_type='text/plain')
                    response['Content-Disposition'] = 'attachment; filename=Mendel,MD Variants Export.txt'
                    writer = csv.writer(response, delimiter='\t', quoting=csv.QUOTE_NONE)    
                writer.writerow(['individual', 'index', 'pos_index', 'chr', 'pos', 'variant_id', 'ref', 'alt', 'qual', 'filter', 'format', 'genotype_col', 'genotype', 'read_depth', 'gene', 'mutation_type', 'genomes1k_maf', 'gnomead_exome_maf', 'gnomead_genome_maf', 'dbsnp_build', 'sift', 'sift_pred', 'polyphen2', 'polyphen2_pred', 'cadd', 'is_at_omim', 'is_at_hgmd', 'hgmd_class', 'hgmd_phen', 'snpeff_effect', 'snpeff_impact', 'snpeff_func_class', 'snpeff_codon_change', 'snpeff_aa_change', 'SIFT_score', 'SIFT_converted_rankscore', 'CADD_raw', 'CADD_raw_rankscore', 'CADD_phred', 'mcap_score', 'mcap_rankscore', 'mcap_pred', 'clinvar_rs', 'clinvar_clnsig', 'clinvar_trait', 'clinvar_golden_stars']) 
                for variant in variants:
                    # print 'variant', variant.index
                    writer.writerow([variant.individual, variant.index, variant.pos_index, variant.chr, variant.pos, variant.variant_id, variant.ref, variant.alt, variant.qual, variant.filter, variant.format, variant.genotype_col, variant.genotype, variant.read_depth, variant.gene, variant.mutation_type, variant.genomes1k_maf, variant.gnomead_exome_maf, variant.gnomead_genome_maf, variant.dbsnp_build, variant.sift, variant.sift_pred, variant.polyphen2, variant.polyphen2_pred, variant.cadd, variant.is_at_omim, variant.is_at_hgmd, variant.hgmd_class, variant.hgmd_phen, variant.snpeff_effect, variant.snpeff_impact, variant.snpeff_func_class, variant.snpeff_codon_change, variant.snpeff_aa_change, variant.SIFT_score, variant.SIFT_converted_rankscore, variant.CADD_raw, variant.CADD_raw_rankscore, variant.CADD_phred, variant.mcap_score, variant.mcap_rankscore, variant.mcap_pred, variant.clinvar_rs, variant.clinvar_clnsig, variant.clinvar_trait, variant.clinvar_golden_stars, ])
                return response
